...
Suppose we have data points that correspond to either class 1 or class 2. The Support Vector Machine (SVM) \cite{RefWorks:122} uses a separating hyperplane to distinguish between data points that belong to class 1 and class 2. Using Statistical Learning Theory, by Vapnik \textit{et. al} \cite{RefWorks:122}, the SVM finds the hyperplane with the largest margin between the two classes.
...
This section would explore the literature of feature selection, and discuss certain algorithms and methodologies employed in the literature. The following terms will be used throughout this paper:
\begin{itemize}
...
Overall, there are several benefits associated with feature selection that are often mentioned in literature:
\begin{itemize}
  ...
  \item \textbf{Improvements in classification accuracy.} More features might, due to experimental faults or the nature of the data, lead to more noise in our data set \cite{RefWorks:163, RefWorks:197}. Having a more compact subset to train our classifier might improve the performance of the classifier \cite{RefWorks:174}.
\end{itemize}
...
\begin{itemize}
\item \textbf{Filters} rank features with either a univariate or multivariate measure. The former evaluates each feature individually, and selects only the top ranking features, while the latter evaluates a subset of features. No learning is involved in filters; they are independent of the classifier \cite{RefWorks:216}. Interaction between features is also not considered (see section \ref{bg:fs:relevance} on relevance and redundancy).

\item \textbf{Wrappers} regard the learning algorithm as a black box, and evaluate features based on the classification score of the learner. Wrappers usually incur more computational cost due to the cross validation procedure (training and prediction steps) when evaluating the features. 
...
\item \textbf{Hybrid} methods are usually a combination of the above approaches.

\end{itemize}

These methods have their own merits and drawbacks. For example, as filters are independent of the classifier, they would be computationally more efficient. However, most filters are univariate, and thus might not take into account feature redundancy (see section \ref{bg:fs:relevance}). One can use multivariate filters which consider subsets, but these incur more computational cost.

On the other hand, although wrappers are reported to produce better classification accuracy than filters \cite{RefWorks:163}, they are closely associated with the choice of learning algorithm, and might not generalise to other classifiers. One can also consider employing a ``two-stage'' process, such as in \cite{RefWorks:216}, where a filter is used to remove the unimportant features, and a classifier-dependent method can then be used to further refine the remaining features.
...
\begin{itemize}
  ...
  \item \textbf{Best performance}: The algorithm terminates when the performance of $S$ exceeds a specified threshold.
  ...
  \item \textbf{Balance between performance and subset size}: Consider both classifier performance and $k$.
\end{itemize}

It seems that from most works in feature selection, authors often specify a fixed value of $k$. Furthermore, in \cite{RefWorks:119} by Tang \textit{et. al}, experiments on different datasets conclude that their algorithm might be a good choice for a small number of samples, with large $M$ and $k$. Furthermore, they pointed out that $k$ must be defined beforehand. The experimenters set this number to be 100 for all the datasets that were explored, but later recommend different values of $k$ for two of the datasets that were used.

This suggests that $k$ is dependent on the dataset. If $k$ is not a constraint of the problem (e.g. in bioinformatics problems, we can specify that we just require $k$ of the most informative genes), we could perform a grid search. However, due to time and computational constraints, we will probably not be able to perform a grid search on all the features (i.e. $k \in X = [0, M]$), but only on an interval of $X$. So, even if we find an optimal $k$ in that interval of $X$, it is most likely a local optimum.
...
\begin{itemize}
  ...
  \item Two parents can perform a \textbf{crossover} to produce a pair of potentially fitter offsprings. There are several strategies for the crossover operator, such as one-point \cite{RefWorks:201}, two-point \cite{RefWorks:202, RefWorks:204} and uniform crossover.
  ...
  \item The \textbf{selection} process selects the fittest chromosome(s) of a population. Depending on the strategy that one employs, the selected chromosome(s) will either be selected as parents for crossover, or they will be retained and moved to the next generation (\textit{elitism/cloning}  strategy \cite{RefWorks:202, RefWorks:212}). One can also select the chromosomes with the worst fitness for crossover, in hope that this will raise the overall fitness of the population \cite{RefWorks:203}.
  
\end{itemize}

GA's encapsulate the concept of \textit{survival of the fittest}, as the fitter a chromosome is, the more likely it is either copied to the next generation, or selected as a parent for crossover \cite{RefWorks:209}. Furthermore, Kudo and Sklansky also recommend GA's for ``large-scale'' problems that involve more than 50 features \cite{RefWorks:210}.
...
Moreover, different strategies can be used in certain parts of the GA. For example, in performing the \textit{crossover}, the parents can either be the solutions with the worst fitness \cite{RefWorks:203}, or selected in proportion to their fitness \cite{RefWorks:205}. The mutation rate, which often takes a small value (e.g. 1 to 15\%) \cite{RefWorks:206}, say $m$, will uniformly choose $m$ percent of a chromosome, and these elements would have their bits inverted.
...
The $t$-statistic is a commonly used heuristic to quantify the importance of each feature with respect to the target labels. As mentioned in section \ref{bg:fs:classification}, the $t$-test is a filter method, which iterates through each of the feature and compute a $t$ statistic:
\begin{align*}
insert formula here
\end{align*}

The $t$ statistic and the $p$ values can be computed using \texttt{scipy.stats.ttest\_ind} in the \texttt{scipy} \cite{scipy} software. For example, we can calculate the $t$ statistic between two groups $a$ and $b$. Using this method, the null hypothesis is that the means of $a$ and $b$ ($\mu_a$ and $\mu_b$ respectively) are equal. That is,
\begin{align*}
H_0: \mu_a = \mu_b \quad \text{vs.} \quad H_1: \mu_a \neq \mu_b
\end{align*}

\texttt{ttest\_ind} will then perform a two-sided test, and return the value of the $t$ statistic and its $p$ value.

In the context of feature selection, for each feature, we have to group the samples according to its classification under this feature. This means that group $a$ will be those samples whose classification is 1, say, and group $b$ will be those samples whose classification is 0. Under the null hypothesis $H_0$, the means of these two groups will be the same. That is, the feature values do not have any impact on the classification of the samples. A low $p$ value means that we have enough evidence to reject $H_0$, that is, we can be certain that this feature has a significant contribution to the classification.

As discussed in section \ref{bg:fs:terminating}, once we have the $p$ values of all the features, we can decide how to use these values to terminate the feature selection algorithm. First, we can select the first $k$ features that have the \textit{lowest} $p$ values (i.e. most certain that $\mu_a \neq \mu_b$). Second, we can choose a certain threshold and discard those features whose $p$ values \textit{exceed} this threshold.

Since this method is a filter method, we would expect this method to complete within a much shorter time than the other methods that will be described and explained below.
...
Lastly, the parameters used in the algorithm implemented are summarised in the table below:
...
\chapter{Results}
% Include insights and discussions.
...
Furthermore, as mentioned in section \ref{bg:fs:ga}, there are many parameters in genetic algorithms that need to be tuned, and it might be enlightening to experiment with parameters that are different from the ones used in this project.

As mentioned in section \ref{bg:fs:mi}, mutual information (MI) is a highly popular measure to quantify the relationship between features and the target class (relevance) and between features themselves (redundancy). As our data set uses continuous variables, it is difficult to estimate probabilities involving these variables. The Parzen Window \cite{RefWorks:183} seems to be a feasible way to estimate these probabilities. A logical extension will be to use MI with the Parzen Window to quantify relevance and redundancy. This can then be combined with genetic algorithms.
