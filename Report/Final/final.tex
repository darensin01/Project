...
\end{tikzpicture}
\caption{The initial population. Suppose that 5 of the solutions are chosen to participate in the tournament. In this case, $N_t=5$. Suppose the dotted entry wins the tournament.}
\label{bg:ga:tour_a}
\end{subfigure}\\[2em]
...
\end{tikzpicture}
\caption{A diagram illustrating 5-fold cross-validation. In each iteration, 20\% of the data is ``held-out'' as the test set (grey boxes), while the rest is used as the training set (white boxes). The overall cross-validation score is the average classification accuracy across all 5 iterations.}
\label{data:cv}
\end{figure}
...
\KwOut{Subset $S$ ($S \subset D$) such that the features in $S$ produce the best fitness.}
\BlankLine
\Begin{
...
$e \longleftarrow $ Select top $p_e$ of solutions in $gen\_solution$. \;
$r \longleftarrow $ Select next $p_r$ of solutions in $gen\_solution$. \;
Perform $mutate$ on each solution in $r$. \;
$next\_gen\_solution \longleftarrow next\_gen\_solution \cup \left\lbrace r,e \right\rbrace$ \;
\BlankLine
...
Evaluate fitness of $gen\_solution$. \;
